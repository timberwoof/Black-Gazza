    ...
    link_message(integer sender_num, integer signal, string message, key incoming_id) {
        if (signal == 1000) {
            ...
            if (message == "TIMER MODE") {
                ...
                llDialog(incoming_id,"1 Please enter the duration:\n\nEntered: "+string2stringtime(MESSAGE),keypad,KEYPAD_CHANNEL);                
            } else if (message == "CLOCK MODE") {
                ENTRY_STYLE = "CLOCK MODE";
                llDialog(incoming_id,"1 Please enter the time:\n\nEntered: "+string2stringtime(MESSAGE),keypad,KEYPAD_CHANNEL);                
            } else {    
                llDialog(incoming_id,"1 Please enter the combination.\n\nEntered: "+MESSAGE,keypad,KEYPAD_CHANNEL);
            }
        }
    }
    ...
    listen(integer heard_channel, string name, key id, string message) {
        ...
        if (message == "E") {
            if (ENTRY_STYLE == "TIMER MODE") {
                llMessageLinked(LINK_THIS,1002,stringtime2integertime(MESSAGE),id);
            } else if (ENTRY_STYLE == "CLOCK MODE") {
                llMessageLinked(LINK_THIS,1003,stringtime2integertime(MESSAGE),id);
            } else {
                llMessageLinked(LINK_THIS,1001,MESSAGE,id);
            }
        } else if (message == "C") {
            MESSAGE = "";
            if (ENTRY_STYLE == "TIMER MODE") {
                llDialog(id,"1 Please enter the duration:\n\nEntered: "+string2stringtime(MESSAGE),keypad,KEYPAD_CHANNEL);
            } else if (ENTRY_STYLE == "CLOCK MODE") {
                llDialog(id,"1 Please enter the time:\n\nEntered: "+string2stringtime(MESSAGE),keypad,KEYPAD_CHANNEL);                
            } else {
                llDialog(id,"1 Please enter the combination.\n\nEntered: "+MESSAGE,keypad,KEYPAD_CHANNEL);
            }
        } else {
            MESSAGE += message;
            if (ENTRY_STYLE == "TIMER MODE") {
